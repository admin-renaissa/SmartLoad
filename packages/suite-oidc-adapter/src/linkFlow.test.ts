...
function claims(partial: Partial<AccessTokenClaims> & Pick<AccessTokenClaims, 'sub' | 'email' | 'org_id'>): AccessTokenClaims {
...
describe('completeAccountLink', () => {
  ...
  it('rejects email mismatch', () => {
    ...
    if (!result.ok) assert.equal(result.error, 'link_email_mismatch');
  });
});
