import { resolveAuthPhase, shouldBlockLocalLogin, shouldShowLocalLoginOnClient, suiteCutoverMessage, SUITE_CUTOVER_CODE, } from '@renverse/auth-sdk';
import { LINK_USER_COOKIE, LOCAL_SESSION_COOKIE } from './localAuth.js';
async function authPhaseForRequest(mode, req, resolveTenantCutover) {
...
export function linkConflictUrl(accountsOrigin, params) {
...
function readLocalUserId(req) {
    const raw = req.cookies?.[LOCAL_SESSION_COOKIE];
    return typeof raw === 'string' && raw.trim() ? raw.trim() : undefined;
}
export function registerMigrationRoutes(opts) {
    const { router, mode, appDisplayName, appKey, accountsOrigin, store, defaultTenantId, } = opts;
...
<p style="margin-top:1.5rem;font-size:0.875rem"><a href="${accountsOrigin}/login">RenVerse Accounts</a></p>
    ...
    router.post('/renverse/link/start', (req, res) => {
        ...
        if (store.countByEmailInTenant(local.email, local.tenantId) > 1) {
            res.status(403).json({
                error: 'link_conflict',
                message: 'More than one local account matches this email. Ask your admin to link accounts.',
                email: local.email,
                appKey,
...
export function renderLinkBannerHtml() {
