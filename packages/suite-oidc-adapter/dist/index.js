...
import { createOidcClient, hasApp, hasAddon, isFlagEnabled, RenverseAuthError, validateAccessToken, resolveAuthPhase, } from '@renverse/auth-sdk';
import { createInMemoryLocalAuthStore, LINK_USER_COOKIE, } from './localAuth.js';
import { completeAccountLink } from './linkFlow.js';
import { registerMigrationRoutes, linkConflictUrl } from './migration.js';
const APP_DISPLAY = {
    renexus: 'Renexus',
    renbooks: 'RenBooks',
    renorc: 'RenOrc',
...
export function seedDemoLocalUser(store, appKey, tenantId) {
...
export function createSuiteOidcRouter(opts) {
    const router = Router();
    const client = createOidcClient(opts.config);
    const mode = process.env.RENVERSE_MODE || 'standalone';
    const appDisplayName = opts.appDisplayName || APP_DISPLAY[opts.config.appKey] || opts.config.appKey;
    const defaultTenantId = opts.defaultTenantId || `tenant_demo_${opts.config.appKey}`;
    const accountsOrigin = opts.accountsOrigin ||
    ...
    router.get('/auth/callback', async (req, res, next) => {
        try {
            ...
            if (opts.config.entitlement === 'addon') {
                if (!hasAddon(session.claims, opts.config.appKey)) {
                    throw new RenverseAuthError('oidc_addon_missing', `${opts.config.appKey} missing from addons[]`);
                }
            }
            else if (!hasApp(session.claims, opts.config.appKey)) {
                throw new RenverseAuthError('oidc_entitlement_missing', `${opts.config.appKey} missing`);
            }
            ...
            if (linkUserId) {
                ...
                if (!link.ok) {
                    ...
                    res.status(403).type('html').send(`<p>We could not link this account (${link.error}). Contact your admin.</p>`);
                    return;
                }
            }
            ...
            if (e instanceof RenverseAuthError) {
                res.status(e.code === 'oidc_entitlement_missing' || e.code === 'oidc_addon_missing'
                    ? 403
                    : 400).json({
                    error: e.code,
                    message: e.message,
...
export function suiteModeEnabled() {
 ...
 */
export function requireSuiteEntitlement(appKey) {
    return async (req, res, next) => {
        ...
        try {
            let session = req.renverseSession || oidc.getSession(req);
            if (!session) {
...
export { createInMemoryLocalAuthStore, } from './localAuth.js';
