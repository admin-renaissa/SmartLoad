...
describe('issa user copy', () => {
  ...
  it('hides tool ids from members and keeps them for ops', () => {
    ...
    assert.equal(
      member.headline,
      'Couldn’t finish in RenBooks. Try again or open the app.',
    );
  ...
  it('scrubs leaked ids and error codes from assistant text', () => {
    const raw =
      'The tool "renbooks.balance.summary" failed with callback_failed.';
    ...
    const prose = sanitizeIssaUserText(
      'Unfortunately, the tool "renbooks.balance.summary" was unable to provide the cash position summary due to a callback failure error.',
    );
    ...
    const leftover = sanitizeIssaUserText(
      'According to the tool results provided, the "renorc.agent.run_status" returned an error code of "callback_failed".',
    );
