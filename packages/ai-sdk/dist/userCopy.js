...
const ERROR_CODE_RE = /\b(?:callback_failed|not_found|not_entitled|confirm_required|forbidden|http_\d{3})\b/gi;
export function issaAppLabel(appOrTool) {
...
export function issaToolUserCopy(opts) {
    ...
    if (opts.audience === 'ops') {
        ...
        if (opts.ok === true)
            return { headline: `Done in ${app}`, detail: id };
        if (opts.ok === false) {
            return {
...
export function sanitizeIssaUserText(text) {
    return text
...
export function issaComposeFacts(results) {
