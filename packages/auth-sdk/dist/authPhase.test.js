...
import { resolveAuthPhase, shouldBlockLocalLogin, shouldShowLocalLoginOnClient, isSuiteAuthCutoverPhase, isSuiteAuthCutoverMisconfigured, } from './authPhase.js';
describe('authPhase', () => {
    ...
    afterEach(() => {
        ...
        if (prevPhase === undefined)
            ...
            process.env.RENVERSE_SUITE_AUTH_PHASE = prevPhase;
        if (prevLegacy === undefined)
            delete process.env.RENVERSE_IDENTITY_ONLY;
        else
            process.env.RENVERSE_IDENTITY_ONLY = prevLegacy;
    });
    ...
    it('ignores env and tenant cutover flags', () => {
        assert.equal(resolveAuthPhase({ mode: 'suite', envPhase: 'cutover' }), 'dual');
        assert.equal(resolveAuthPhase({ mode: 'suite', tenantCutover: true }), 'dual');
    });
    ...
    it('never blocks local login', () => {
        assert.equal(shouldBlockLocalLogin({
            mode: 'suite',
            authPhase: 'cutover',
            tenantCutover: true,
        }), false);
        assert.equal(shouldBlockLocalLogin({ mode: 'standalone', tenantCutover: true }), false);
    });
    it('always shows local login UI in suite', () => {
        assert.equal(shouldShowLocalLoginOnClient({ suiteMode: true, authPhase: 'dual' }), true);
        assert.equal(shouldShowLocalLoginOnClient({ suiteMode: true, authPhase: 'cutover' }), true);
    });
});
