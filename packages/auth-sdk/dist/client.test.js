...
import { hasApp, hasAddon, getOrgId, orgContextHeaders, assertOrgMatch, isFlagEnabled, ORG_CONTEXT_HEADER, RenverseAuthError, } from './index.js';
const claims = {
    sub: 'usr_1',
...
describe('auth-sdk 0.2 helpers', () => {
    ...
    it('assertOrgMatch throws oidc_org_mismatch', () => {
        assert.throws(() => assertOrgMatch(claims, 'org_other'), (e) => e instanceof RenverseAuthError && e.code === 'oidc_org_mismatch');
    ...
    it('isFlagEnabled suite defaults', () => {
        ...
        if (prevConnect === undefined)
            ...
            process.env.RENVERSE_CONNECT_URL = prevConnect;
        if (prevIssa === undefined)
            delete process.env.RENVERSE_ISSA_URL;
        else
            process.env.RENVERSE_ISSA_URL = prevIssa;
    });
});
